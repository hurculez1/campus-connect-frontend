      ...
      onSuccess: (data, variables, context) => {
        queryClient.setQueryData(['messages', matchId], (old) => {
          ...
          if (exists) {
            ...
            // Socket hasn't arrived yet, replace temp with real ID but keep timestamp
    ...
    newSocket.on('new_message', (data) => {
      if (String(data.matchId) === String(matchId)) {
        queryClient.setQueryData(['messages', matchId], (old) => {
          ...
          const filtered = old.filter(m => !(String(m.id).startsWith('temp-') && m.content === msg.content));
          return [...filtered, msg];
        });
      ...
      // Also background refresh the matches list so the sidebar/lobby updates too
